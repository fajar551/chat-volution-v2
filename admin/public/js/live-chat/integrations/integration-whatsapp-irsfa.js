...
const showFormConnect = async () => {
    ...
    fetch("https://waserverlive.genio.id/init", requestOptions)
      .then((result) =>{
        $(".btn-connect").empty();
        $(".btn-connect").append(`
...
socketIrsfa.on('qrCode', async (data) => {
...
const disconectWa = () => {
    ...
    socketIrsfa.emit("disconnect.whatsapp");
};

socketIrsfa.on("disconnect.whatsappresult", async (response) => {
    if(response.success){
        await checkStatus();
        Toast.fire({
            icon: "success",
