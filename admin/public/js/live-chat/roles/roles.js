...
var settings = {
    url: `${window.base_url_live}/api/agent/roles/list`,
    method: "POST",
    headers: {
...
var dataTable = $("#example").DataTable({
    ...
    columns: [
        ...
        //     'sClass': "text-center",
        //     "mRender": function (data) {
        //         return `<a href="#" onclick="deleteFunction(${data.id})" class="text-danger"><i class="ri-delete-bin-7-line font-size-18" style="position: relative;top: 4px;"></i> Delete</a>`
        //     }
        // }
    ],
});
...
function deleteFunction(id) {
    Swal.fire({
        ...
        if (result.value) {
            ...
            settings.url = `${window.base_url_live}/api/agent/roles`;
            $.ajax(settings).done(function (response) {
                Toast.fire({
                    icon: "success",
                    title: "Delete role successfuly!",
                });
...
function changeStatus(id) {
    ...
    settings.url = `${window.base_url_live}/api/agent/roles/status`;
    $(".page-loader").removeAttr("style");
    $.ajax(settings).done(function (response) {
        dataTable.ajax.reload();
