...
/* save session to server socket v2 */
axios(configV2);
        ...
        if (is_menu == "private") {
            ...
            axios(config)
                .then(async (response) => {
                    ...
                    localStorage.setItem('prevMessage', data.message);
                    getContactChat();
                    /* scroll to bottom */
                    setTimeout(() => {
                        let el = document.getElementById(
...
const sendCaption = () => {
    if (!Boolean(this.replyMessage)) {
        let inputChat = document.querySelector("#input_caption");
        let message = inputChat.innerHTML;
        message = message
            .replace(/<div>/gi, "")
            .replace(/<\/div>/gi, "")
...
const updateTitleApps = () => {
    ...
    let oldIcon = '/assets/images/favicon.ico';
    let newIcon = '/assets/images/favicon-dot-red.ico';
    function changeNewFavicon() {
        document.getElementById('favicon').href = newIcon;
    }

    function changeOldFavicon() {
        document.getElementById('favicon').href = oldIcon;
    }
