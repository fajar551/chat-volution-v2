...
const checkSoundPermission = () => {
    const config = {
        ...
        url: `${window.base_url_live}/api/settings/me`,
    };
    const status_notify = localStorage.getItem("status");
    if (status_notify == 0) {
...
const notificationWithSound = () => {
    const notify_status = localStorage.getItem("status");
    const urlAudio = `${base_url_live}/assets/sound/bell.mp3`;
    const audio = new Audio(urlAudio);
...
$(document).on("contextmenu", ".list-chat-private", function (event) {
    ...
    let hasRead = el.data("hasread");

    $(".action-list-private-chat").append(`
        <a class="dropdown-item font-weight-bold pointer" onclick="detailChat('${selectChatID}','',${hasRead})">Detail Chat</a>
    ...
    if (dataRes.length > 0) {
        dataRes.forEach((valRes) => {
            ...
            $("#dt_contact_chat").append(`
                    <li class="pointer list-chat-private chat chat-${valRes.chat_id}" data-hasread="${valRes.has_read}" data-chatid="${valRes.chat_id}" onclick="detailChat('${valRes.chat_id}','',${valRes.has_read})">
                        <a>
                            <div class="media">
            ...
            if (!Boolean(valRes.has_read)) {
                $(`.text-${valRes.chat_id}`).addClass("text-bolder");
                $(`#date-${valRes.chat_id}`).attr("style", "margin-bottom:5px");
            } else {
                $(`#date-${valRes.chat_id}`).removeAttr("style");
                $(`.text-${valRes.chat_id}`).removeClass("text-bolder");
            }
        });
        if (this.dataDetail != undefined) {
...
$(document).on("contextmenu", ".list-chat-group", function (event) {
    ...
    $(".action-list-group-chat").append(`
        <a class="dropdown-item font-weight-bold pointer" onclick="detailChat('${selectChatID}','${idchat}',${hasRead})">Detail Chat Group</a>
    ...
    if (dataRes.length > 0) {
        ...
        dataRes.forEach((valRes) => {
            ...
            $("#dt_group_chat").append(`
                <li class="pointer list-chat-group chat chat-${valRes.chat_id}" data-chatid="${valRes.chat_id}" data-hasread="${valRes.has_read}" data-idChat="${valRes.id}" onclick="detailChat('${valRes.chat_id}','${valRes.id}',${valRes.has_read})">
                    <a>
                        <div class="media">
...
const getInfoGroupFromDetailChat = (id, from) => {
    ...
    axios(config)
        .then(async (response) => {
            ...
            if (
                this.dataDetail.has_read !== true ||
                this.dataDetail.has_read !== "true"
            ) {
                getGroupChat();
            }

            $(`#date-${this.dataDetail.chat_id}`).removeAttr("style");
            $(`.text-${this.dataDetail.chat_id}`).removeClass("text-bolder");
...
const detailChat = async (chat_id, id, has_read = true) => {
    ...
    axios(config)
        .then(async (response) => {
            ...
            if (this.dataDetail.chat_type_name.toLowerCase() == "group") {
                ...
                if (has_read !== true || has_read !== "true") {
                    getContactChat();
                }

                $(`#date-${chat_id}`).removeAttr("style");
                $(`.text-${chat_id}`).removeClass("text-bolder");
        ...
        $("#header-right-content").append(`
            ...
            <div class="dropdown chat-noti-dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right">
                    ...
                    <a class="dropdown-item pointer" onclick="getListWebhook('${this.dataDetailGroupChat2.id}')">
...
const chatConversation = async (receiver) => {
    ...
    axios(config)
        .then((response) => {
            ...
            getContactChat();
            setTimeout(() => {
                detailChat(dataRes.chat_id, receiver);
            }, 1200);
        })
        .catch((err) => {
            console.warn("conversation action response:", err.response);
        });
};
...
const saveWebhook = () => {
    const name = $("#name_webhook").val();
    var data = new FormData();
    const file = document.querySelector("#file_webhook").files[0];
    data.append("name", name);
    data.append("image", file);
    data.append("chat_group_id", this.dataDetailGroupChat2.id);
    const config = {
        method: "POST",
        headers: {
            Authorization: `Bearer ${dataAuth.token}`,
        ...
        mimeType: "multipart/form-data",
        data,
        url: `${window.base_url_live}/api/agent/chat-webhook`,
    };

    axios(config)
        .then((response) => {
            $(".loader-list-webhook").show();
            setTimeout(function () {
                return Toast.fire({
                    icon: "success",
                    title: "Success save webhook"
                });
            }, 1100);
            getListWebhook(this.dataDetailGroupChat2.id)
            $(".loader-list-webhook").hide();
        })
        .catch((error) => {
            console.warn(error);
        });
...
const detailWebhook = (id) => {
    $(".loader-detail-list").show();
    $("#detailWebhook").hide();
    $("#addWebhook").hide();
    $("#show-add-webhook").hide();
    $(".webhook-list").css("max-height", "20vh");
    $("#id_webhook_detail").val(id);
    const config = {
        method: "GET",
        headers: {
            Authorization: `Bearer ${dataAuth.token}`,
        ...
        url: `${window.base_url_live}/api/agent/chat-webhook/${id}`,
    };
    axios(config)
        .then((response) => {
            ...
            $("#show-add-webhook").hide();
            $(".webhook-list").css("max-height", "20vh");
            $("#name_webhook_detail").val(resData.name);
            $("#file_webhook_detail").val(resData.image);
            $("#detailWebhook").show();
...
const updateWebhook = () => {
    const id_group = this.dataDetailGroupChat2.id;
    const name = $("#name_webhook_detail").val();
    const file = document.querySelector("#file_webhook_detail");
    const id = $("#id_webhook_detail").val();
    var data = new FormData();
    data.append("name", name);
    data.append("id", id);
    if (file.files[0] == undefined) {
        data.append("image", $("#file_webhook_detail").val());
    } else {
         data.append("image", file.files[0]);
    }
    data.append("chat_group_id", id_group);
    $("#detailWebhook").hide();
    $("#show-add-webhook").show();
    $(".loader-list-webhook").show();
    const config = {
        method: "POST",
        headers: {
            Authorization: `Bearer ${dataAuth.token}`,
            "Content-Type": file.files[0] == undefined ? "application/json" : "multipart/form-data",
            "X-Requested-With": "xmlhttprequest",
        },
        data,
        url: `${window.base_url_live}/api/agent/chat-webhook/update`,
    };
    axios(config)
        .then((response) => {
            $(".loader-list-webhook").hide();
            setTimeout(function () {
                return Toast.fire({
                    icon: "success",
                    title: "Success update webhook"
                });
            }, 1100);
            getListWebhook(id_group);

            $(".loader-list-webhook").hide();
            $(".webhook-list").css("max-height", "50vh");
        })
        .catch((error) => {
            console.warn(error);
        });
...
const deleteWebhook = (id) => {
    ...
    const id_group = this.dataDetailGroupChat2.id;
    Swal.fire({
        text: "Are you sure remove this webhook?",
        icon: "info",
        showCancelButton: true,
        allowEscapeKey: false,
        allowOutsideClick: false,
...
/* modalManage Webhook */
const getListWebhook = (chat_group_id) => {
    $("#modalManageWebhook").modal("show");
    $(".loader-list-webhook").show();
    const elListWebhook = document.getElementById("listWebhook");
    elListWebhook.innerHTML = "";
    const config = {
        method: "GET",
        headers: {
            Authorization: `Bearer ${dataAuth.token}`,
        ...
        url: `${window.base_url_live}/api/agent/chat-webhook/list/${chat_group_id}`,
    };

    $(document).ready(function() {
        $("#show-add-webhook").click(function() {
            $("#addWebhook").show();
            $(".webhook-list").css("max-height", "20vh");
            $(this).hide();
        });

        $("#cancel-button, #add-button").click(function() {
            $("#show-add-webhook").show();
            $("#addWebhook").hide();
            $("#detailWebhook").hide();
            $(".webhook-list").css("max-height", "50vh");
    ...
    axios(config)
        .then((response) => {
            ...
            if (dataRes.length < 1) {
                $(".loader-list-webhook").hide();
                return (elListWebhook.innerHTML = `
                    <li class="d-flex mt-3">
                        <div class="conversation-list">
                            <div class="ctext-wrap">
                                <div class="ctext-wrap-content">
            ...
            dataRes.forEach((webhook) => {
                ...
                return (elListWebhook.innerHTML += `
                    <li class="mt-3" style="width: 100%">
                        <div class="webhook-render-list">
                            <div class="ctext-wrap">
                                <div class="ctext-wrap-content d-flex justify-content-between">
                                    <div class="d-inline-flex">
                                        <div class="webhook-image">
                                            <img src="${image}" class="rounded-circle avatar-xs img-object-fit-cover" alt="">
                                        </div>
                                        <div class="webhook-content text-truncate ml-3" style="max-width: 300px">
                                            <h5 class="mb-0 font-weight-bold">
                                                ${webhook.name}
                                            </h5>
                                            <p id="webhook_url" class="mb-0">
                                                ${webhook.webhook_url}
                                            </p>
                                        </div>
                                    </div>
                                    <div class="webhook-action">
                                        <button class="btn btn-outline-gray" onclick="copyWebhook()"><i class="fas fa-copy font-size-20"></i></button>
                                        <div class="dropdown chat-noti-dropdown">
                                            <button class="btn dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                <i class="fas fa-ellipsis-v font-size-20 mr-4"></i>
                                            </button>
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <a class="dropdown-item text-primary pointer" onclick="detailWebhook(${webhook.id})">
                                                    <span>Update Webhook</span>
                                                </a>
                                                <a class="dropdown-item text-danger pointer" onclick="deleteWebhook(${webhook.id})">
...
/* end modal Manage Webhook */

function copyWebhook() {
  // Get the text field
  let webhookUrl = document.getElementById("webhook_url");

  navigator.clipboard.writeText(webhookUrl.textContent.trim());

  return Toast.fire({
        icon: "success",
        title: "Success copy webhook url"
    });
}
...
String.prototype.insertHtmlAtCursor = (html) => {
    if (window.getSelection().baseNode.parentNode.id == "input_caption") {
        var range, node;
        if (window.getSelection && window.getSelection().getRangeAt) {
            range = window.getSelection().getRangeAt(0);
            node = range.createContextualFragment(html);
        ...
        const elInputChatId = document.getElementById("input_caption");
        elInputChatId.innerHTML = elInputChatId.innerHTML + html;
    }
};
...
$("#input_chat").keydown((e) => {
    let inputChat = document.querySelector("[contenteditable]");
    let msgContent = inputChat.innerHTML;
    // Save previous message to local storage
    localStorage.setItem('currentMessage', msgContent);

    // Retrieve previous message from local storage
    let prevMessage = localStorage.getItem('prevMessage');

    if(prevMessage === msgContent){
        return
    }else{
        if (e.which === 32) {
        ...
        if (enterCount === 1 && prevMessage !== msgContent){
            e.preventDefault();
            sendMessage();
            setTimeout(() => {
                enterCount = 0;
        ...
        /* input message and button send message */
        document.querySelector(".send-chat").addEventListener("click", (e) => {
            const button = document.querySelector(".send-chat");
            let prevMessage = localStorage.getItem('prevMessage');
            let currentMessage = localStorage.getItem('currentMessage');
            enterCount += 1;
            if (enterCount === 1 && prevMessage !== currentMessage){
                e.preventDefault();
                sendMessage();
...
function sendImage(){
    ...
    let file = document.querySelector('#file-upload').value.replace(/C:\\fakepath\\/i, '');
