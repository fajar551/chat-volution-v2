...
settings.method = "GET";
settings.url = "/api/agent/oauth-client/list";
...
const selectText = (node) => {
    ...
    $(".btn-copy")
        .attr("data-original-title", "Copied: " + node.textContent)
        .tooltip("show");
...
const deleteToken = (id, domain) => {
    Swal.fire({
        ...
        if (result.value) {
            ...
            settings.url = `/api/agent/oauth-client/${id}`;
            $.ajax(settings)
                .done(function (response) {
                    ...
                    socket.emit("manageDomain", dataOauth);
                    Toast.fire({
                        icon: "success",
...
$(function () {
    ...
    $("#btn-save").submit(function (e) {
        ...
        // let parseUrl = psl.parse($("#domain").val());
        if (!getUrl) {
            $("#err_domain").empty();
            $("#err_domain").append(
    ...
    $("#page-copy").append(
        `
            ...
            <button type="button" class="btn-sm btn-secondary btn-copy" data-toggle="tooltip" data-placement="top"
                title="Copy To Clipboard" onclick="copy('add')" onmouseout="outCursor()">
