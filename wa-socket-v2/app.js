  ...
  try {
    ...
    try {
      ...
      await WhatsAppMessage.create({
        ...
        instance: 'wa2' // Set instance to wa2 for wa-socket-v2
      });
...
wa2Router.get('/api/whatsapp/messages-by-instance/:instance', async (req, res) => {
  try {
...
wa2Router.get('/api/whatsapp/messages/:phone', async (req, res) => {
  try {
    ...
    if (dbMessages.length > 0) {
      console.log('📋 Messages order from DB (wa2Router):');
    ...
    const transformedMessages = dbMessages.map((msg, index) => {
      ...
      return {
        ...
        instance: msg.instance || 'wa2', // Include instance from database
...
app.get('/api/whatsapp/unread-counts', async (req, res) => {
  try {
    ...
    // Simple query to get unread messages (only from open chats, wa2 instance)
    const unreadMessages = await WhatsAppMessage.findAll({
      where: {
        direction: 'incoming',
        is_read: false,
        chat_status: 'open', // Only count unread from open chats
        instance: 'wa2' // Only count unread messages from wa2 instance
      },
...
// This endpoint is duplicated from wa2Router for direct access
...
// Get unread count for a specific phone number (wa2 instance)
wa2Router.get('/api/whatsapp/unread-count/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
...
wa2Router.get('/api/whatsapp/assigned-status/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
...
wa2Router.post('/api/whatsapp/assign-chat/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
...
// Mark messages as read for a specific phone number (wa2 instance)
wa2Router.post('/api/whatsapp/mark-read/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    console.log('📖 Marking messages as read for phone (wa2):', phone);
    ...
    // Find unread messages (only for wa2 instance)
    const unreadMessages = await WhatsAppMessage.findAll({
      where: {
        from_number: fromNumberWhere,
        direction: 'incoming',
        instance: 'wa2', // Only mark messages from wa2 instance
        [Op.or]: [
          { is_read: false },
          { is_read: 0 },
          { is_read: null }
    ...
    console.log('📖 Unread messages (wa2, before update):', unreadMessages.length);
      ...
      const [affected] = await WhatsAppMessage.update(
        { is_read: true },
        {
          where: {
            ...
            instance: 'wa2' // Ensure only wa2 messages are updated
          }
        }
      );
      totalAffected = affected;
      console.log('📖 Rows affected (wa2, by id):', totalAffected);
    }
    ...
    if (totalAffected === 0) {
      const [affected] = await WhatsAppMessage.update(
        { is_read: true },
        {
          where: {
            from_number: fromNumberWhere,
            direction: 'incoming',
            instance: 'wa2', // Only update wa2 messages
            [Op.or]: [
              { is_read: false },
              { is_read: 0 },
              { is_read: null }
      ...
      totalAffected = affected;
      console.log('📖 Rows affected (wa2, by condition):', totalAffected);
    }
    ...
    console.error('❌ Error marking messages as read (wa2):', error);
    res.status(500).json({ error: 'Failed to mark messages as read' });
  }
});

// Update chat status to closed (for solve chat functionality)
wa2Router.post('/api/whatsapp/close-chat/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    ...
    // Update all messages for this phone to closed status and set is_pending = 0 (only for wa2 instance)
    const [affectedRows] = await WhatsAppMessage.update(
      { chat_status: 'closed', is_pending: false },
      {
        where: {
          [Op.and]: [
            ...
            { instance: 'wa2' } // Only update messages from wa2 instance
          ]
        }
      }
    );

    console.log('✅ Updated', affectedRows, 'messages to closed status for phone:', phone, '(wa2 instance)');
...
// Pending chat endpoint for wa2 instance
wa2Router.post('/api/whatsapp/pending-chat/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    ...
    // Update all messages for this phone to pending status (only for wa2 instance)
    ...
    console.log('✅ Updated', affectedRows, 'messages to pending status for phone:', phone, '(wa2 instance)');
...
wa2Router.get('/api/whatsapp/closed-chats', async (req, res) => {
  try {
    ...
    // Get phones with closed messages and their latest closed message (only for wa2 instance)
    const closedMessages = await WhatsAppMessage.findAll({
      where: {
        chat_status: 'closed',
        instance: 'wa2' // Only get closed chats from wa2 instance
      },
      attributes: [
        'from_number',
        'to_number',
    ...
    if (phonesArray.length > 0) {
      ...
      phoneToLatestClosed.forEach((data, phone) => {
        closedPhonesOnly.push({
          ...
          status: 'closed',
          instance: data.instance || 'wa2', // Include instance from database
      ...
      const openFromNumbers = await WhatsAppMessage.findAll({
        where: {
          ...
          chat_status: 'open',
          instance: 'wa2' // Only check open messages from wa2 instance
        },
...
// Get chats list from database (aggregated from whatsapp_messages) for wa2
wa2Router.get('/api/whatsapp/chats', async (req, res) => {
  try {
    console.log('📥 Fetching WhatsApp chats list from database (wa2)');
    ...
    console.log(`📊 Found ${chats.length} chats in database (wa2)`);
    ...
    console.error('❌ Error fetching chats from database (wa2):', error);
    return res.status(500).json({
      success: false,
...
app.get('/api/whatsapp/closed-chats', async (req, res) => {
  try {
    ...
    if (phonesArray.length > 0) {
      // Find phones that have open messages (check both from_number and to_number, only for wa2 instance)
      const openFromNumbers = await WhatsAppMessage.findAll({
        where: {
          from_number: { [Op.in]: phonesArray },
          chat_status: 'open',
...
app.post('/api/whatsapp/pending-chat/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    // Get instance from query parameter or default to wa2
    const instance = req.query.instance || req.headers['x-instance'] || 'wa2';
...
wa2Router.get('/api/whatsapp/agent/:agentId', async (req, res) => {
  try {
