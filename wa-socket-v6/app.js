...
// Router untuk /wa6 path - duplicate semua routes dengan prefix /wa6
const wa6Router = express.Router();

// Copy semua routes ke wa6Router
wa6Router.get('/', (req, res) => {
  res.send('Hello World from fajar di wa socket v2?! ');
});

wa6Router.get('/status', async (req, res) => {
  try {
...
wa6Router.post('/send', async (req, res) => {
  try {
    ...
    try {
      ...
      await WhatsAppMessage.create({
        ...
        instance: 'wa6' // Set instance to wa6 for wa-socket-v6
      });
...
wa6Router.get('/logout', async (req, res) => {
  try {
    await client.logout();
...
wa6Router.get('/scan', (req, res) => {
  res.sendFile(__dirname + '/scan.html');
});

wa6Router.get('/cleanup-auth', (req, res) => {
  try {
...
wa6Router.get('/messages', (req, res) => {
  try {
...
wa6Router.get('/api/whatsapp/messages-by-instance/:instance', async (req, res) => {
  try {
...
wa6Router.get('/api/whatsapp/messages/:phone', async (req, res) => {
  try {
    ...
    if (dbMessages.length > 0) {
      console.log('📋 Messages order from DB (wa6Router):');
    ...
    const transformedMessages = dbMessages.map((msg, index) => {
      ...
      return {
        ...
        instance: msg.instance || 'wa6', // Include instance from database
...
wa6Router.get('/messages/:phone', (req, res) => {
  try {
...
// API routes
wa6Router.get('/api/api-keys', async (req, res) => {
  try {
...
wa6Router.get('/api/api-keys/:service', async (req, res) => {
  try {
...
wa6Router.post('/api/api-keys', async (req, res) => {
  try {
...
wa6Router.post('/api/api-keys/test/:service', async (req, res) => {
  try {
    const { service } = req.params;
    const apiKey = await ApiKey.findOne({
      where: { service: service, is_active: true }
...
// Serve socket.io.js for /wa6 (Apache ProxyPass removes /wa6 prefix)
...
// Use wa6Router for /wa6 path
app.use('/wa6', wa6Router);
...
app.get('/api/whatsapp/unread-counts', async (req, res) => {
  try {
    ...
    // Simple query to get unread messages (only from open chats, wa6 instance)
    const unreadMessages = await WhatsAppMessage.findAll({
      where: {
        direction: 'incoming',
        is_read: false,
        chat_status: 'open', // Only count unread from open chats
        instance: 'wa6' // Only count unread messages from wa6 instance
      },
...
// This endpoint is duplicated from wa6Router for direct access
...
// Get unread count for a specific phone number (wa6 instance)
wa6Router.get('/api/whatsapp/unread-count/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
...
wa6Router.get('/api/whatsapp/assigned-status/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
...
wa6Router.post('/api/whatsapp/assign-chat/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
...
// Mark messages as read for a specific phone number (wa6 instance)
wa6Router.post('/api/whatsapp/mark-read/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    console.log('📖 Marking messages as read for phone (wa6):', phone);
    ...
    // Find unread messages (only for wa6 instance)
    const unreadMessages = await WhatsAppMessage.findAll({
      where: {
        from_number: fromNumberWhere,
        direction: 'incoming',
        instance: 'wa6', // Only mark messages from wa6 instance
        [Op.or]: [
          { is_read: false },
          { is_read: 0 },
          { is_read: null }
    ...
    console.log('📖 Unread messages (wa6, before update):', unreadMessages.length);
      ...
      const [affected] = await WhatsAppMessage.update(
        { is_read: true },
        {
          where: {
            ...
            instance: 'wa6' // Ensure only wa6 messages are updated
          }
        }
      );
      totalAffected = affected;
      console.log('📖 Rows affected (wa6, by id):', totalAffected);
    }
    ...
    if (totalAffected === 0) {
      const [affected] = await WhatsAppMessage.update(
        { is_read: true },
        {
          where: {
            from_number: fromNumberWhere,
            direction: 'incoming',
            instance: 'wa6', // Only update wa6 messages
            [Op.or]: [
              { is_read: false },
              { is_read: 0 },
              { is_read: null }
      ...
      totalAffected = affected;
      console.log('📖 Rows affected (wa6, by condition):', totalAffected);
    }
    ...
    console.error('❌ Error marking messages as read (wa6):', error);
    res.status(500).json({ error: 'Failed to mark messages as read' });
  }
});

// Update chat status to closed (for solve chat functionality)
wa6Router.post('/api/whatsapp/close-chat/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    ...
    // Update all messages for this phone to closed status and set is_pending = 0 (only for wa6 instance)
    const [affectedRows] = await WhatsAppMessage.update(
      { chat_status: 'closed', is_pending: false },
      {
        where: {
          [Op.and]: [
            ...
            { instance: 'wa6' } // Only update messages from wa6 instance
          ]
        }
      }
    );

    console.log('✅ Updated', affectedRows, 'messages to closed status for phone:', phone, '(wa6 instance)');
...
// Pending chat endpoint for wa6 instance
wa6Router.post('/api/whatsapp/pending-chat/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    ...
    // Update all messages for this phone to pending status (only for wa6 instance)
    ...
    console.log('✅ Updated', affectedRows, 'messages to pending status for phone:', phone, '(wa6 instance)');
...
wa6Router.get('/api/whatsapp/closed-chats', async (req, res) => {
  try {
    ...
    // Get phones with closed messages and their latest closed message (only for wa6 instance)
    const closedMessages = await WhatsAppMessage.findAll({
      where: {
        chat_status: 'closed',
        instance: 'wa6' // Only get closed chats from wa6 instance
      },
      attributes: [
        'from_number',
        'to_number',
    ...
    if (phonesArray.length > 0) {
      ...
      phoneToLatestClosed.forEach((data, phone) => {
        closedPhonesOnly.push({
          ...
          status: 'closed',
          instance: data.instance || 'wa6', // Include instance from database
      ...
      const openFromNumbers = await WhatsAppMessage.findAll({
        where: {
          ...
          chat_status: 'open',
          instance: 'wa6' // Only check open messages from wa6 instance
        },
...
wa6Router.get('/api/whatsapp/chats', async (req, res) => {
  try {
    console.log('📥 Fetching WhatsApp chats list from database (wa6)');
    ...
    console.log(`📊 Found ${chats.length} chats in database (wa6)`);
    ...
    console.error('❌ Error fetching chats from database (wa6):', error);
    return res.status(500).json({
      success: false,
...
app.get('/api/whatsapp/closed-chats', async (req, res) => {
  try {
    ...
    if (phonesArray.length > 0) {
      // Find phones that have open messages (check both from_number and to_number, only for wa6 instance)
      const openFromNumbers = await WhatsAppMessage.findAll({
        where: {
          from_number: { [Op.in]: phonesArray },
          chat_status: 'open',
...
app.post('/api/whatsapp/pending-chat/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    // Get instance from query parameter or default to wa6
    const instance = req.query.instance || req.headers['x-instance'] || 'wa6';
...
wa6Router.get('/api/whatsapp/agent/:agentId', async (req, res) => {
  try {
