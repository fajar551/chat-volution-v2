...
// Router untuk /wa3 path - duplicate semua routes dengan prefix /wa3
const wa3Router = express.Router();

// Copy semua routes ke wa3Router
wa3Router.get('/', (req, res) => {
  res.send('Hello World from fajar di wa socket v2?! ');
});

wa3Router.get('/status', async (req, res) => {
  try {
...
wa3Router.post('/send', async (req, res) => {
  try {
    ...
    try {
      ...
      await WhatsAppMessage.create({
        ...
        instance: 'wa3' // Set instance to wa3 for wa-socket-v3
      });
...
wa3Router.get('/logout', async (req, res) => {
  try {
    await client.logout();
...
wa3Router.get('/scan', (req, res) => {
  res.sendFile(__dirname + '/scan.html');
});

wa3Router.get('/cleanup-auth', (req, res) => {
  try {
...
wa3Router.get('/messages', (req, res) => {
  try {
...
wa3Router.get('/api/whatsapp/messages-by-instance/:instance', async (req, res) => {
  try {
...
wa3Router.get('/api/whatsapp/messages/:phone', async (req, res) => {
  try {
    ...
    if (dbMessages.length > 0) {
      console.log('📋 Messages order from DB (wa3Router):');
    ...
    const transformedMessages = dbMessages.map((msg, index) => {
      ...
      return {
        ...
        instance: msg.instance || 'wa3', // Include instance from database
...
wa3Router.get('/messages/:phone', (req, res) => {
  try {
...
// API routes
wa3Router.get('/api/api-keys', async (req, res) => {
  try {
...
wa3Router.get('/api/api-keys/:service', async (req, res) => {
  try {
...
wa3Router.post('/api/api-keys', async (req, res) => {
  try {
...
wa3Router.post('/api/api-keys/test/:service', async (req, res) => {
  try {
    const { service } = req.params;
    const apiKey = await ApiKey.findOne({
      where: { service: service, is_active: true }
...
// Serve socket.io.js for /wa3 (Apache ProxyPass removes /wa3 prefix)
...
// Use wa3Router for /wa3 path
app.use('/wa3', wa3Router);
    ...
    if (isRatingMessage && ratingValue) {
      ...
      const insertSql = `
        INSERT INTO whatsapp_messages (
          ...
          chat_session_id, status, is_read, chat_status, assigned_to, instance,
          createdAt, updatedAt
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `;
      ...
      try {
        ...
        console.log('✅ Message saved using Sequelize fallback, ID:', savedMessage.id, 'Rating:', ratingValue);
      }
    } else {
      ...
      console.log('✅ Message saved to database with ID:', savedMessage.id, assignedTo ? `(assigned to ${assignedTo})` : '');
    }
  } catch (error) {
...
client.on('message', async (message) => {
  try {
    ...
    // AI response disabled - only manual replies allowed
...
app.get('/api/whatsapp/unread-counts', async (req, res) => {
  try {
    ...
    // Simple query to get unread messages (only from open chats, wa3 instance)
    const unreadMessages = await WhatsAppMessage.findAll({
      where: {
        direction: 'incoming',
        is_read: false,
        chat_status: 'open', // Only count unread from open chats
        instance: 'wa3' // Only count unread messages from wa3 instance
      },
...
// This endpoint is duplicated from wa3Router for direct access
...
// Get unread count for a specific phone number (wa3 instance)
wa3Router.get('/api/whatsapp/unread-count/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
...
wa3Router.get('/api/whatsapp/assigned-status/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
...
wa3Router.post('/api/whatsapp/assign-chat/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
...
// Mark messages as read for a specific phone number (wa3 instance)
wa3Router.post('/api/whatsapp/mark-read/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    console.log('📖 Marking messages as read for phone (wa3):', phone);
    ...
    // Find unread messages (only for wa3 instance)
    const unreadMessages = await WhatsAppMessage.findAll({
      where: {
        from_number: fromNumberWhere,
        direction: 'incoming',
        instance: 'wa3', // Only mark messages from wa3 instance
        [Op.or]: [
          { is_read: false },
          { is_read: 0 },
          { is_read: null }
    ...
    console.log('📖 Unread messages (wa3, before update):', unreadMessages.length);
      ...
      const [affected] = await WhatsAppMessage.update(
        { is_read: true },
        {
          where: {
            ...
            instance: 'wa3' // Ensure only wa3 messages are updated
          }
        }
      );
      totalAffected = affected;
      console.log('📖 Rows affected (wa3, by id):', totalAffected);
    }
    ...
    if (totalAffected === 0) {
      const [affected] = await WhatsAppMessage.update(
        { is_read: true },
        {
          where: {
            from_number: fromNumberWhere,
            direction: 'incoming',
            instance: 'wa3', // Only update wa3 messages
            [Op.or]: [
              { is_read: false },
              { is_read: 0 },
              { is_read: null }
      ...
      totalAffected = affected;
      console.log('📖 Rows affected (wa3, by condition):', totalAffected);
    }
    ...
    console.error('❌ Error marking messages as read (wa3):', error);
    res.status(500).json({ error: 'Failed to mark messages as read' });
  }
});

// Update chat status to closed (for solve chat functionality)
wa3Router.post('/api/whatsapp/close-chat/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    ...
    // Update all messages for this phone to closed status (only for wa3 instance)
    const [affectedRows] = await WhatsAppMessage.update(
      { chat_status: 'closed' },
      {
        where: {
          [Op.and]: [
            ...
            { instance: 'wa3' } // Only update messages from wa3 instance
          ]
        }
      }
    );

    console.log('✅ Updated', affectedRows, 'messages to closed status for phone:', phone, '(wa3 instance)');
...
wa3Router.get('/api/whatsapp/closed-chats', async (req, res) => {
  try {
    ...
    // Get phones with closed messages and their latest closed message (only for wa3 instance)
    const closedMessages = await WhatsAppMessage.findAll({
      where: {
        chat_status: 'closed',
        instance: 'wa3' // Only get closed chats from wa3 instance
      },
      attributes: [
        'from_number',
        'to_number',
    ...
    if (phonesArray.length > 0) {
      ...
      phoneToLatestClosed.forEach((data, phone) => {
        closedPhonesOnly.push({
          ...
          status: 'closed',
          instance: data.instance || 'wa3', // Include instance from database
      ...
      const openFromNumbers = await WhatsAppMessage.findAll({
        where: {
          ...
          chat_status: 'open',
          instance: 'wa3' // Only check open messages from wa3 instance
        },
...
wa3Router.get('/api/whatsapp/chats', async (req, res) => {
  try {
    console.log('📥 Fetching WhatsApp chats list from database (wa3)');
    ...
    console.log(`📊 Found ${chats.length} chats in database (wa3)`);
    ...
    console.error('❌ Error fetching chats from database (wa3):', error);
    return res.status(500).json({
      success: false,
...
app.get('/api/whatsapp/closed-chats', async (req, res) => {
  try {
    ...
    if (phonesArray.length > 0) {
      // Find phones that have open messages (check both from_number and to_number, only for wa3 instance)
      const openFromNumbers = await WhatsAppMessage.findAll({
        where: {
          from_number: { [Op.in]: phonesArray },
          chat_status: 'open',
...
wa3Router.get('/api/whatsapp/agent/:agentId', async (req, res) => {
  try {
