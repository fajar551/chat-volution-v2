...
// Router untuk /wa5 path - duplicate semua routes dengan prefix /wa5
const wa5Router = express.Router();

// Copy semua routes ke wa5Router
wa5Router.get('/', (req, res) => {
  res.send('Hello World from fajar di wa socket v2?! ');
});

wa5Router.get('/status', async (req, res) => {
  try {
...
wa5Router.post('/send', async (req, res) => {
  try {
    ...
    try {
      ...
      await WhatsAppMessage.create({
        ...
        instance: 'wa5' // Set instance to wa5 for wa-socket-v5
      });
...
wa5Router.get('/logout', async (req, res) => {
  try {
    await client.logout();
...
wa5Router.get('/scan', (req, res) => {
  res.sendFile(__dirname + '/scan.html');
});

wa5Router.get('/cleanup-auth', (req, res) => {
  try {
...
wa5Router.get('/messages', (req, res) => {
  try {
...
wa5Router.get('/api/whatsapp/messages-by-instance/:instance', async (req, res) => {
  try {
...
wa5Router.get('/api/whatsapp/messages/:phone', async (req, res) => {
  try {
    ...
    if (dbMessages.length > 0) {
      //('📋 Messages order from DB (wa5Router):');
    ...
    const transformedMessages = dbMessages.map((msg, index) => {
      ...
      return {
        ...
        instance: msg.instance || 'wa5', // Include instance from database
...
wa5Router.get('/messages/:phone', (req, res) => {
  try {
...
// API routes
wa5Router.get('/api/api-keys', async (req, res) => {
  try {
...
wa5Router.get('/api/api-keys/:service', async (req, res) => {
  try {
...
wa5Router.post('/api/api-keys', async (req, res) => {
  try {
...
wa5Router.post('/api/api-keys/test/:service', async (req, res) => {
  try {
    const { service } = req.params;
    const apiKey = await ApiKey.findOne({
      where: { service: service, is_active: true }
...
// Serve socket.io.js for /wa5 (Apache ProxyPass removes /wa5 prefix)
...
// Use wa5Router for /wa5 path
app.use('/wa5', wa5Router);
...
app.get('/api/whatsapp/unread-counts', async (req, res) => {
  try {
    ...
    // Simple query to get unread messages (only from open chats, wa5 instance)
    const unreadMessages = await WhatsAppMessage.findAll({
      where: {
        direction: 'incoming',
        is_read: false,
        chat_status: 'open', // Only count unread from open chats
        instance: 'wa5' // Only count unread messages from wa5 instance
      },
...
// This endpoint is duplicated from wa5Router for direct access
...
// Get unread count for a specific phone number (wa5 instance)
wa5Router.get('/api/whatsapp/unread-count/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
...
wa5Router.get('/api/whatsapp/assigned-status/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
...
wa5Router.post('/api/whatsapp/assign-chat/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
...
// Mark messages as read for a specific phone number (wa5 instance)
wa5Router.post('/api/whatsapp/mark-read/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    //('📖 Marking messages as read for phone (wa5):', phone);
    ...
    // Find unread messages (only for wa5 instance)
    const unreadMessages = await WhatsAppMessage.findAll({
      where: {
        from_number: fromNumberWhere,
        direction: 'incoming',
        instance: 'wa5', // Only mark messages from wa5 instance
        [Op.or]: [
          { is_read: false },
          { is_read: 0 },
          { is_read: null }
    ...
    //('📖 Unread messages (wa5, before update):', unreadMessages.length);
      ...
      const [affected] = await WhatsAppMessage.update(
        { is_read: true },
        {
          where: {
            ...
            instance: 'wa5' // Ensure only wa5 messages are updated
          }
        }
      );
      totalAffected = affected;
      //('📖 Rows affected (wa5, by id):', totalAffected);
    }
    ...
    if (totalAffected === 0) {
      const [affected] = await WhatsAppMessage.update(
        { is_read: true },
        {
          where: {
            from_number: fromNumberWhere,
            direction: 'incoming',
            instance: 'wa5', // Only update wa5 messages
            [Op.or]: [
              { is_read: false },
              { is_read: 0 },
              { is_read: null }
      ...
      totalAffected = affected;
      //('📖 Rows affected (wa5, by condition):', totalAffected);
    }
    ...
    console.error('❌ Error marking messages as read (wa5):', error);
    res.status(500).json({ error: 'Failed to mark messages as read' });
  }
});

// Update chat status to closed (for solve chat functionality)
wa5Router.post('/api/whatsapp/close-chat/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    ...
    // Update all messages for this phone to closed status and set is_pending = 0 (only for wa5 instance)
    const [affectedRows] = await WhatsAppMessage.update(
      { chat_status: 'closed', is_pending: false },
      {
        where: {
          [Op.and]: [
            ...
            { instance: 'wa5' } // Only update messages from wa5 instance
          ]
        }
      }
    );

    //('✅ Updated', affectedRows, 'messages to closed status for phone:', phone, '(wa5 instance)');
...
// Pending chat endpoint for wa5 instance
wa5Router.post('/api/whatsapp/pending-chat/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    ...
    // Update all messages for this phone to pending status (only for wa5 instance)
    ...
    console.log('✅ Updated', affectedRows, 'messages to pending status for phone:', phone, '(wa5 instance)');
...
wa5Router.get('/api/whatsapp/closed-chats', async (req, res) => {
  try {
    ...
    // Get phones with closed messages and their latest closed message (only for wa5 instance)
    const closedMessages = await WhatsAppMessage.findAll({
      where: {
        chat_status: 'closed',
        instance: 'wa5' // Only get closed chats from wa5 instance
      },
      attributes: [
        'from_number',
        'to_number',
    ...
    if (phonesArray.length > 0) {
      ...
      phoneToLatestClosed.forEach((data, phone) => {
        closedPhonesOnly.push({
          ...
          status: 'closed',
          instance: data.instance || 'wa5', // Include instance from database
      ...
      const openFromNumbers = await WhatsAppMessage.findAll({
        where: {
          ...
          chat_status: 'open',
          instance: 'wa5' // Only check open messages from wa5 instance
        },
...
wa5Router.get('/api/whatsapp/chats', async (req, res) => {
  try {
    //('📥 Fetching WhatsApp chats list from database (wa5)');
    ...
    //(`📊 Found ${chats.length} chats in database (wa5)`);
    ...
    console.error('❌ Error fetching chats from database (wa5):', error);
    return res.status(500).json({
      success: false,
...
app.get('/api/whatsapp/closed-chats', async (req, res) => {
  try {
    ...
    if (phonesArray.length > 0) {
      // Find phones that have open messages (check both from_number and to_number, only for wa5 instance)
      const openFromNumbers = await WhatsAppMessage.findAll({
        where: {
          from_number: { [Op.in]: phonesArray },
          chat_status: 'open',
...
app.post('/api/whatsapp/pending-chat/:phone', async (req, res) => {
  try {
    const { phone } = req.params;
    // Get instance from query parameter or default to wa5
    const instance = req.query.instance || req.headers['x-instance'] || 'wa5';
...
wa5Router.get('/api/whatsapp/agent/:agentId', async (req, res) => {
  try {
